...
class MapProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        junction_itr=iter(junction.outgoing_links)
        for outgoing_link in junction_itr:
            yield OperatorResult(successor_state=MapState(outgoing_link.target), operator_cost=outgoing_link.distance)
